...
public class InAppWebView: WKWebView, UIScrollViewDelegate, WKUIDelegate,
    ...
    static let HOTWIRE_FILE_CHOOSER_CHANNEL = "hotwire_native_flutter/file_chooser"
    ...
    public func webView(_ webView: WKWebView,
        ...
        guard let channel = hotwireFileChooserChannel() else {
        ...
        let args: [String: Any] = [
            "acceptTypes": [],
            "allowMultiple": parameters.allowsMultipleSelection,
            "capture": false
        ]

        channel.invokeMethod("showFileChooser", arguments: args) { result in
            guard let paths = result as? [String] else {
                completionHandler(nil)
                return
            }
    ...
    private func hotwireFileChooserChannel() -> FlutterMethodChannel? {
        guard let registrar = plugin?.registrar else {
            return nil
        }
        return FlutterMethodChannel(
            name: InAppWebView.HOTWIRE_FILE_CHOOSER_CHANNEL,
            binaryMessenger: registrar.messenger()
        )
    }
