...
public class InAppWebViewChromeClient extends WebChromeClient implements PluginRegistry.ActivityResultListener, ActivityResultListener {
  ...
  protected static final String HOTWIRE_FILE_CHOOSER_CHANNEL =
  ...
  private MethodChannel hotwireFileChooserChannel;
  ...
  protected void openFileChooser(ValueCallback<Uri> filePathCallback, String acceptType) {
    if (handleHotwireFileChooserLegacy(filePathCallback, acceptType, null)) {
      return;
    }
  ...
  protected void openFileChooser(ValueCallback<Uri> filePathCallback) {
    if (handleHotwireFileChooserLegacy(filePathCallback, "", null)) {
      return;
    }
  ...
  protected void openFileChooser(ValueCallback<Uri> filePathCallback, String acceptType, String capture) {
    if (handleHotwireFileChooserLegacy(filePathCallback, acceptType, capture)) {
      return;
    }
  ...
  public boolean onShowFileChooser(WebView webView, ValueCallback<Uri[]> filePathCallback, FileChooserParams fileChooserParams) {
    ...
    if (handleHotwireFileChooser(acceptTypes, allowMultiple, captureEnabled, filePathCallback)) {
      return true;
    }
    return startPickerIntent(filePathCallback, acceptTypes, allowMultiple, captureEnabled);
  }

  private boolean handleHotwireFileChooserLegacy(ValueCallback<Uri> callback, String acceptType, String capture) {
    if (callback == null) {
      return false;
    }
    String[] acceptTypes = acceptType == null ? new String[]{} : new String[]{acceptType};
    boolean captureEnabled = capture != null && !capture.isEmpty();
    return handleHotwireFileChooser(acceptTypes, false, captureEnabled, new ValueCallback<Uri[]>() {
      @Override
      public void onReceiveValue(Uri[] value) {
        Uri first = value != null && value.length > 0 ? value[0] : null;
        callback.onReceiveValue(first);
      }
    });
  }

  private boolean handleHotwireFileChooser(String[] acceptTypes, boolean allowMultiple, boolean capture,
                                           ValueCallback<Uri[]> filePathCallback) {
    MethodChannel channel = getHotwireFileChooserChannel();
    if (channel == null) {
      return false;
    }
    Map<String, Object> args = new HashMap<>();
    args.put("acceptTypes", Arrays.asList(acceptTypes));
    args.put("allowMultiple", allowMultiple);
    args.put("capture", capture);
    channel.invokeMethod("showFileChooser", args, new MethodChannel.Result() {
      @Override
      public void success(Object result) {
        Uri[] uris = parseHotwireFileChooserResult(result);
        webView.post(() -> filePathCallback.onReceiveValue(uris));
      }

      @Override
      public void error(String errorCode, String errorMessage, Object errorDetails) {
        webView.post(() -> filePathCallback.onReceiveValue(null));
      }
  ...
  private MethodChannel getHotwireFileChooserChannel() {
    if (hotwireFileChooserChannel != null) {
      return hotwireFileChooserChannel;
    }
    if (webView == null || webView.plugin == null) {
      return null;
    }
    hotwireFileChooserChannel = new MethodChannel(webView.plugin.messenger, HOTWIRE_FILE_CHOOSER_CHANNEL);
    return hotwireFileChooserChannel;
  }

  private Uri[] parseHotwireFileChooserResult(Object result) {
